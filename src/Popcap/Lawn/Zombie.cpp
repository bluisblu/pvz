...
Plant *Zombie::FindCatapultTarget()
{
    ...
    {
        if (mRow == aPlant->mRow && mX >= aPlant->mX + 100 && !aPlant->NotOnGround() &&
            !aPlant->IsSpiky())
        {
...
bool Zombie::HasYuckyFaceImage()
{
    ...
    if (mZombieType == ZOMBIE_NORMAL || mZombieType == ZOMBIE_TRAFFIC_CONE ||
        mZombieType == ZOMBIE_PAIL || mZombieType == ZOMBIE_FLAG || mZombieType == ZOMBIE_DOOR ||
        mZombieType == ZOMBIE_DUCKY_TUBE || mZombieType == ZOMBIE_DANCER ||
        mZombieType == ZOMBIE_BACKUP_DANCER || mZombieType == ZOMBIE_NEWSPAPER ||
...
Reanimation *Zombie::AddAttachedReanim(int thePosX, int thePosY, ReanimationType theReanimType)
{
    ...
    theY = (float)(thePosY + mY);
    theX = (float)(thePosX + mX);
    ...
    if (v6)
    {
        ...
        AttachReanim(mAttachmentID, v6, theOffsetX, theOffsetY);
    }
    return v7;
}
...
void Zombie::StartEating()
{
    ...
    if (!mIsEating)
    {
        mZombiePhase = this->mZombiePhase;
        mIsEating    = 1;
        if (mZombiePhase != PHASE_DIGGER_TUNNELING)
        {
            if (mZombiePhase == PHASE_LADDER_CARRYING)
            {
                PlayZombieReanim("anim_laddereat", REANIM_LOOP, 20, 0.0);
            }
            else if (mZombiePhase == PHASE_NEWSPAPER_MAD)
            {
                PlayZombieReanim("anim_eat_nopaper", REANIM_LOOP, 20, 0.0);
            }
            else
            {
                if (mZombieType != ZOMBIE_SNORKEL)
                    PlayZombieReanim("anim_eat", REANIM_LOOP, 20, 0.0);
...
void Zombie::StopEating()
{
    ...
    {
        mApp          = this->mApp;
        mBodyReanimID = this->mBodyReanimID;
        mIsEating     = 0;
        v3            = mApp->ReanimationTryToGet(mBodyReanimID);
        if (mZombiePhase != PHASE_DIGGER_TUNNELING)
        {
            if (v3 && mZombieType != ZOMBIE_SNORKEL)
                StartWalkAnim(20);
            if (mShieldType == SHIELDTYPE_DOOR)
                ShowDoorArms(1);
            UpdateAnimSpeed();
...
void Zombie::DieNoLoot()
{
    ...
    mApp->RemoveReanimation(this->mBodyReanimID);
    mApp->RemoveReanimation(this->mMoweredReanimID);
    mApp->RemoveReanimation(this->mSpecialHeadReanimID);
    mDead = 1;
    TrySpawnLevelAward();
    if (mZombieType == ZOMBIE_BOBSLED)
        BobsledDie();
...
void Zombie::ApplyChill(bool theIsIceTrap)
{
    ...
    if (CanBeChilled())
    {
        if (!this->mChilledCounter)
            mApp->PlayFoley(FOLEY_FROZEN);
        v3 = 1000;
        if (theIsIceTrap)
            v3 = 2000;
        mChilledCounter = this->mChilledCounter;
        if (v3 > mChilledCounter)
            mChilledCounter = v3;
        this->mChilledCounter = mChilledCounter;
        UpdateAnimSpeed();
    }
}

void __thiscall Zombie::DragUnder()
{
    Reanimation *v2; // eax
    int v3;          // ecx
    int v4;          // edx

    this->mZombieHeight = HEIGHT_DRAGGED_UNDER;
    StopEating();
    v2 = mApp->ReanimationTryToGet(mBodyReanimID);
    if (v2)
    {
        v3 = 0;
        if (v2->mDefinition->mTrackCount > 0)
        {
            v4 = 0;
            do
            {
                v2->mTrackInstances[v4].mIgnoreClipRect = 0;
                ++v3;
                ++v4;
            } while (v3 < v2->mDefinition->mTrackCount);
        }
    }
}
