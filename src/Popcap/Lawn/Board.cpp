...
bool Board::NeedSaveGame()
{
    ...
    if (mode == GAMEMODE_CHALLENGE_ICE || mode == GAMEMODE_CHALLENGE_ZEN_GARDEN ||
        mode == GAMEMODE_TREE_OF_WISDOM)
    {
        return false;
    }
...
bool Board::IsFlagWave(int theWaveNumber)
{
    ...
    int waves;
    if (!mApp->IsFirstTimeAdventureMode() || (waves = mNumWaves, waves > 10))
    {
        waves = 10;
        // mNumWaves = waves;
    }
    return theWaveNumber % waves == waves + -1;
}
...
bool Board::IsPlantInCursor()
{
    ...
    if (mCursorType == CURSOR_TYPE_PLANT_FROM_BANK ||
        mCursorType == CURSOR_TYPE_PLANT_FROM_USABLE_COIN ||
        mCursorType == CURSOR_TYPE_PLANT_FROM_GLOVE ||
        mCursorType == CURSOR_TYPE_PLANT_FROM_DUPLICATOR ||
...
{
    if (theZombieType != ZOMBIE_SNORKEL && theZombieType != ZOMBIE_DOLPHIN_RIDER)
    {
        return false;
    }
    return true;
...
bool Board::CanInteractWithBoardButtons()
{
    ...
    if (!mPaused && mApp->GetDialogCount() <= 0 &&
        (mCursorType = mCursorObject->mCursorType, mCursorType != CURSOR_TYPE_PLANT_FROM_BANK) &&
        mCursorType != CURSOR_TYPE_PLANT_FROM_USABLE_COIN &&
        mCursorType != CURSOR_TYPE_PLANT_FROM_GLOVE &&
        mCursorType != CURSOR_TYPE_PLANT_FROM_DUPLICATOR &&
        mCursorType != CURSOR_TYPE_PLANT_FROM_WHEEL_BARROW && mCursorType != CURSOR_TYPE_SHOVEL &&
        mCursorType != CURSOR_TYPE_WATERING_CAN && mCursorType != CURSOR_TYPE_FERTILIZER &&
        mCursorType != CURSOR_TYPE_BUG_SPRAY && mCursorType != CURSOR_TYPE_PHONOGRAPH &&
        mCursorType != CURSOR_TYPE_CHOCOLATE && mCursorType != CURSOR_TYPE_GLOVE &&
        mCursorType != CURSOR_TYPE_MONEY_SIGN && mCursorType != CURSOR_TYPE_WHEEELBARROW &&
        mCursorType != CURSOR_TYPE_TREE_FOOD && mBoardFadeOutCounter < 0 &&
        mChallenge->mChallengeState != STATECHALLENGE_ZEN_FADING &&
        mApp->mCrazyDaveState == CRAZY_DAVE_OFF)
    {
        return true;
    }
    return false;
...
bool Board::HasProgressMeter()
{
    if (mApp->mGameMode == GAMEMODE_CHALLENGE_BEGHOULED ||
        mApp->mGameMode == GAMEMODE_CHALLENGE_BEGHOULED_TWIST || mApp->IsFinalBossLevel() ||
...
bool Board::ProgressMeterHasFlags()
{
    ...
    if (mApp->IsWhackAZombieLevel() || mApp->IsFinalBossLevel() ||
        ...
        mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIQUARIUM || mApp->IsSlotMachineLevel() ||
...
{
    if (mApp->IsScaryPotterLevel() && mNextSurvivalStageCounter > 0 && mApp->mCrazyDaveState)
    {
        return true;
    }
    return false;
...
int Board::GetNumSeedsInBank()
{
    ...
    if (mApp->mGameMode == GAMEMODE_PUZZLE_I_ZOMBIE_5 ||
        mApp->mGameMode == GAMEMODE_PUZZLE_I_ZOMBIE_6 ||
...
{
    if (mBackground == BACKGROUND_2_NIGHT || mBackground == BACKGROUND_4_FOG ||
        mBackground == BACKGROUND_6_BOSS || mBackground == BACKGROUND_MUSHROOM_GARDEN ||
...
bool Board::StageHasGraveStones()
{
    ...
    if (mApp->mGameMode == GAMEMODE_CHALLENGE_POGO_PARTY ||
        mApp->mGameMode == GAMEMODE_CHALLENGE_BEGHOULED ||
        mApp->mGameMode == GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
        mApp->mGameMode == GAMEMODE_CHALLENGE_PORTAL_COMBAT ||
...
bool Board::StageHasZombieWalkInFromRight()
{
    ...
    if (mApp->IsFinalBossLevel() || mApp->IsIZombieLevel() || mApp->IsSquirrelLevel() ||
...
int Board::GetSeedPacketPositionX(int theIndex)
{
    ...
    // mNumPackets = mSeedBank->mNumPackets;
    if (mApp->IsSlotMachineLevel())
...
unsigned int Board::SeedNotRecommendedForLevel(SeedType theSeedType)
{
    ...
    if (theSeedType == SEED_INSTANT_COFFEE &&
        (mBackground == BACKGROUND_2_NIGHT || mBackground == BACKGROUND_4_FOG ||
         mBackground == BACKGROUND_6_BOSS || mBackground == BACKGROUND_MUSHROOM_GARDEN ||
         mBackground == BACKGROUND_ZOMBIQUARIUM))
    {
        flags |= 0x80u;
    ...
    if (theSeedType == SEED_FLOWERPOT && mBackground != BACKGROUND_5_ROOF &&
    ...
    if (mBackground != BACKGROUND_3_POOL && mBackground != BACKGROUND_4_FOG &&
        (theSeedType == SEED_LILYPAD || theSeedType == SEED_TANGLEKELP ||
