...
bool LawnHasUsedCheatKeys()
{
    if (!gLawnApp || !gLawnApp->mPlayerInfo)
        return false;
    return gLawnApp->mPlayerInfo->mHasUsedCheatKeys != 0;
}
...
void LawnApp::KillStoreScreen()
{
    ...
    {
        this->KillDialog(4);
        Sexy::SexyAppBase::ClearUpdateBacklog(0);
    }
}
...
{
    if (mGameMode >= GAMEMODE_SURVIVAL_NORMAL_STAGE_1 &&
        mGameMode <= GAMEMODE_SURVIVAL_ENDLESS_STAGE_5)
        return true;
...
bool LawnApp::IsArtChallenge()
{
    ...
    if (mGameMode == GAMEMODE_CHALLENGE_ART_CHALLENGE_1 ||
        mGameMode == GAMEMODE_CHALLENGE_ART_CHALLENGE_2 ||
...
bool LawnApp::IsWallnutBowlingLevel()
{
    ...
    if (mGameMode == GAMEMODE_CHALLENGE_WALLNUT_BOWLING ||
        mGameMode == GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2)
        return true;
...
bool LawnApp::UpdateApp()
{
    ...
    if (mLoadingThreadCompleted && !mExitToTop)
        // non-matching: extra `mov ecx, esi` right before the function call
        SexyAppBase::PreDDInterfaceInitHook();
...
{
    return (10 - GetNumTrophies(CHALLENGE_PAGE_SURVIVAL)) +
           (20 - GetNumTrophies(CHALLENGE_PAGE_CHALLENGE)) +
...
bool LawnApp::IsContinuousChallenge()
{
    if (IsArtChallenge() || IsSlotMachineLevel() || IsFinalBossLevel())
        return true;
    if (mGameMode == GAMEMODE_CHALLENGE_BEGHOULED ||
...
bool LawnApp::CanShowAlmanac()
{
    ...
    bool canShow = (mPlayerInfo->mLevel >= 15);
    return canShow;
}
...
bool LawnApp::CanShowStore()
{
    ...
    bool canShow = (mPlayerInfo->mLevel >= 25);
    return canShow;
}
...
bool LawnApp::CanShowZenGarden()
{
    ...
    bool canShow = (mPlayerInfo->mLevel >= 45);
    return canShow;
}
...
SexyString LawnApp::GetMoneyString(int theAmount)
{
    ...
    if (aValue > 999999)
    {
        return Sexy::StrFormat(_S("$%d,%03d,%03d"), aValue / 1000000,
